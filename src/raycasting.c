...
double	calculate_rayon_horizontal(t_game *game, int i)
{
	...
	if (game->ray[0][i].angle < PI) // on regarde vers le NORD 
	{
	...
	else if (game->ray[0][i].angle > PI) // on regarde vers le SUD
	{
	...
	//on check si c'est un mur en ajoutant l'offset tant que ce n'est pas un mur
	while (1)
	{
		if (coll_ray(game, 0, i))
		{
			distance_wall = sqrt(((game->ray[0][i].r.x - game->player.coord.x) * (game->ray[0][i].r.x - game->player.coord.x)) 
					+ (game->ray[0][i].r.y - game->player.coord.y) * (game->ray[0][i].r.y - game->player.coord.y));
			break;
		}
		else
		{
...
double	calculate_rayon_vertical(t_game *game, int i)
{
	...
	if (game->ray[1][i].angle > (PI / 2) && game->ray[1][i].angle < (3 * PI / 2)) // on regarde vers l'OUEST
	{
